...
```{r}
#| echo: true
factor(x, levels = c("apple", "banana", "cat",
                     "dog", "Queen Elizabeth"))
```
...
<!-- + Look at your rendered report before you submit it. -->

<!-- **Data Description Components** -->

<!-- + When describing the data, include **context** as well as the data characteristics.  -->
<!--   + Where did the data come from? What years? Location? -->
...
<!-- Which geographic region sold the most total organic, small Hass avocados in 2017? -->
...
<!-- For all four California regions, plot the proportion of the average Hass avocado sales that are small, large, or extra large and conventional vs. organic. -->
...
`\\w` -- matches any **"word"**  (`\\W` matches any **not** "word") 

`\\d` -- matches any **digit** (`\\D` matches any **not** digit) 

`\\s` -- matches any **whitespace** (`\\S` matches any **not** whitespace) 
