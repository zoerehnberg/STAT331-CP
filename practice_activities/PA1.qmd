...
1. Click [here](https://posit.cloud/content/5704780) to access a **Posit Cloud** project.
  + Note: if you do not have a Posit Cloud account, you will be asked to create one.
  + Note: make sure you save a permanent version!

2. If you have already [installed R, RStudio, and Quarto](https://earobinson95.github.io/stat331-calpoly-text/01-introduction.html#checkin3-install), you can create a new [Quarto document](https://earobinson95.github.io/stat331-calpoly-text/01-introduction.html#notebooks), and copy the [practice activity template](https://raw.githubusercontent.com/zoerehnberg/STAT331-S23/main/practice_activities/PA1.qmd) into the file.
  + Make sure your Quarto file is on Source mode and not Visual mode (top left) when you copy the template.
...
*Be careful not to* **overwrite** *more than you intend. If this happens, go back and run (6) and any subsequent code chunks to start fresh with the top_secret vector.*
...
```{r}
# The code below verifies that there are 352 numbers with values **below** 17
# should be 352!
sum(top_secret < 17)
```
...
```{r}
## Complete the code to carry out step 4
top_secret <-  matrix(top_secret, ncol = ____)
```
...
# Code completed for you
evens <- (1:191)*2
# alternative solution: evens <- seq(from = 2, to = 382, by = 2)
```
...
5. Subtract 100 from all numbers in the 18-24th rows of the 3rd column of your secret message matrix.

```{r}
## Complete the code to carry out step 5
top_secret[18:24, 3] <- top_secret[_____, __] - 100
```

6. Multiply all numbers in the 4th and 5th column of your secret message matrix by 2.

```{r}
## Code to carry out step 6
```

7. Turn your secret message matrix back into a vector.

```{r}
## Complete the code to carry out step 7
top_secret <- ___(top_secret)
```

**Checkpoint:** Headquarters has informed you that at this stage of decoding, all numbers in indices 500 and beyond are above 100. The code below verifies that this is true for your `top_secret` object! 

```{r}
# The code below verifies that indices 500 and beyond have values **below** 100
# should be 0!
sum(top_secret[500:955] >= 100)
```

The following operations on our secret message are completed for you, but make sure to still run these code chunks!
...
```{r}
top_secret <- round(top_secret, digits = 0)
```
...
```{r}
# The code below verifies how many even numbers are in your top_secret vector
# Should be 344!
sum(top_secret %% 2 == 0)
```
